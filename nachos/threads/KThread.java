...
public class KThread {
	...
	public void join() {
		...
		// 		//check the ThreadQueue.java and Scheduler.java description
		...
		if (status != statusFinished) {
			...
			System.out.println(joinCaller.getName());
			joinCaller.sleep();
		}
    ...
    public static void joinTest1 () {
		System.out.println("Join Test 1");
	...
	//Test to see that when a child thread is still running, that the parent
	//does indeed wait properly in the readyQueue before being allowed to run again

	// public static void joinTest2 () {
	// 	System.out.println("Join Test 2");
	// KThread a = new KThread( new Runnable () {
	// 	public void run() {
	// 	    System.out.println("I don't (heart) Nachos!");
	// 		KThread b = new KThread( new Runnable() {
	// 			public void run() {
	// 				System.out.println("I really don't (heart) Nachos!");
	// 			}
	// 		});
	// 		b.setName("B").fork();
	//
	// 		b.join();
	// 		System.out.println("After joining, A should be blocked.");
	// 		System.out.println("is it? " + (a.status == statusBlocked));
	// 		Lib.assertTrue((a.status == statusBlocked), " Expected a to be blocked.");
	// 	}
	//     });
	// a.setName("child1").fork();
	// //a.join();
	//
	// // System.out.println("After joining, A should be put back on readyQueue.");
	// // System.out.println("is it? " + (a.status == statusReady));
	// // Lib.assertTrue((a.status == statusReady), " Expected a to be on readyQueue.");
    // }


	public static void joinTest3 () {
		System.out.println("Join Test 3");
	KThread child1 = new KThread( new Runnable () {
		public void run() {
		    System.out.println("I (heart) Nachos!");
			for (int i = 0; i < 10; i++) {
			    //System.out.println ("a is currently busy...");
			    KThread.currentThread().yield();
			}
	...
	// our solutions to the problems cannot busy wait, our test
	// programs can!
	System.out.println("Im here before join");
	child1.join(); //makes it wait here until child is done
	System.out.println("After joining, child1 should be finished.");
	System.out.println("is it? " + (child1.status == statusFinished));
	Lib.assertTrue((child1.status == statusFinished), " Expected child1 to be finished.");
    }
